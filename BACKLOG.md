...
- [x] P1-208 Implement contest UI. A protected Contest page creates ordered private drafts from canonical catalogue problems; a contest-session route displays exactly the active item, routes to the private workspace/source, accepts only terminal advancement and shows a factual post-session summary without timer, score or penalty claims. Automated UI lifecycle regressions and final quality gates pass.
...
- [x] P1-605 Track training and contest sessions. Training creation records a compact start fact; terminal item changes record owner-scoped factual activity and close the session once every item is completed or skipped. Contest start, completion and skipped-item lifecycle facts are now equally owner-scoped and compact, carrying only session/item identifiers and a canonical problem reference where applicable. Regressions prove activity metadata never retains the private contest title or problem content; final quality gates pass.
...
- [x] P1-703 Create skill dependency graph. Immutable `skill_edge_graph_memberships` bind each dependency edge to a taxonomy snapshot; the migration backfills all valid legacy edges for versions `1.0.0` and `1.1.0`, while the protected Skill Map reads only current-version edges. Deterministic prerequisite DAG validation rejects cycles with their closed path. Database verification, domain/router regressions, visual review and quality gates pass.
- [x] P1-704 Implement mastery calculation. Protected `skills.mastery` deterministically derives only direct owner-scoped solved-problem evidence through approved, version-member skill mappings. Two independently solved mapped problems are required before an estimate; otherwise the result is `insufficient_evidence`, never a fabricated low score. Calculation version, minimum evidence and score remain visible in Skill Map; domain and authorization regressions, visual review and quality gates pass.
...
- [x] P1-803 Account for weak skills and recent exposure. `difficulty-progression-v1` derives a target only after three owner-scoped solved problems with verified canonical difficulty: one 100-point step above their median with a bounded ±200 range. With insufficient evidence it returns no target; target proximity only tie-breaks existing factual adaptive priorities. Domain, protected-router and Training UI regressions pass; the full quality suite has 110 tests.
...
- [x] P1-905 Implement AI-generated personalized contests. A protected server-side structured `claude-haiku-4-5` request receives only up to 24 eligible canonical public candidates plus coarse selection reasons, never private notes, attempts, identity or raw activity. Strict schema and server validation reject malformed JSON, duplicate/unavailable IDs and invalid title/duration values. The UI displays the proposal and applies it to editable fields only after explicit confirmation; it never auto-creates or starts a contest. User-visible feedback explains insufficient eligible problems or a safe generic generation/validation failure. Mocked router/UI contract regressions and final quality gates pass.
- [x] P1-906 Implement contest performance analysis. Protected contest detail now provides `contest-performance-v1`: factual item outcomes, completion percentage, elapsed contest time from start/terminal or server-observation evidence, timed-completion availability and the existing score snapshot. Completed and expired session views expose these facts and explicitly reject rank, rating, skill, problem-quality and performance-label inference. Shared analysis, protected-detail and terminal UI regressions plus final quality gates pass.
- [x] P1-907 Implement Contest Autopsy. Terminal contest detail now projects `contest-autopsy-v1`: a position-ordered owner-authorized trace with stored terminal statuses, canonical problem references and completion elapsed time only when timestamps are valid, plus factual score/timing summary. It is explicitly unavailable while a contest is active or draft; completed and expired session UI exposes the review without inferring a cause, mistake, strategy, rank, rating or skill. Domain, protected-detail, terminal UI and final quality regressions pass.
- [x] P1-908 Implement replay mode. Protected terminal contest detail now projects `contest-replay-v1` solely from the factual autopsy trace; it is unavailable for active/draft sessions. Completed and expired views provide local previous/next frame navigation over recorded ordered states and valid completion clocks only. Replay performs no mutation, re-execution, solution reveal, score/timing alteration or inference. Domain, protected-detail, terminal UI and final quality regressions pass.
...
- [x] P1-1003 Implement progress analysis. Protected `olimp.ai.progressAnalysis` now derives `progress-analysis-v1` solely from the P1-1002 owner-scoped aggregate context, producing status counts, evidence totals, version metadata and explicit insufficiency/limitations rather than a score, trend or prediction. The Progress view renders its facts, safe empty state and non-blocking unavailable state. Domain, router, UI and final quality regressions pass.
- [x] P1-1004 Implement recurring-mistake detection. Protected `olimp.ai.recurringPatterns` now returns `recurring-patterns-v1` from only owner-scoped attempt `state`, `outcome` and `highestHintLevel`, requiring two matching persisted records before reporting an unresolved-outcome, abandoned-attempt or later-stage-hint-use pattern. The Progress view labels these as repeated facts rather than diagnoses, and renders explicit insufficient/no-pattern/unavailable states. Domain, router, UI and final quality regressions pass.
- [x] P1-1005 Implement problem recommendations. Dedicated protected `olimp.ai.problemRecommendations` now derives `problem-recommendations-v1` from real catalogue candidates and owner-scoped progress, active-training, active-contest and solved-difficulty evidence. It excludes terminal/assigned work, provides explicit reasons and insufficiency states, and Training renders a non-blocking card surface with editable preselection handoff. Domain, router, UI and final quality regressions pass.
- [x] P1-1006 Implement training recommendations. Dedicated protected `olimp.ai.trainingRecommendations` now derives `training-recommendations-v1` from real P1-1005 eligible problems and bounded completed-attempt timing evidence, supplies a versioned editable plan and explicit unavailable-duration/no-eligible states. Training renders this plan with an editable title/problem preselection handoff. Domain, router, UI and final quality regressions pass.
- [x] P1-1007 Implement contest analysis. Protected contest detail now exposes `contest-analysis-v1`, a terminal-only projection over persisted performance, scoring and ordered autopsy facts. The terminal review identifies its factual contract and limits; active/draft contests explicitly receive no analysis. Domain regression and final quality suite pass.
- [x] P1-1008 Implement progressive hint system. Protected `olimp.workspace.progressiveGuidance` now reads the server-persisted disclosure level and returns only already approved/revealed hint levels, with an explicit first-hint boundary. Workspace renders a level-labelled recap beside the existing server-enforced next-hint flow. The guidance is replay-only and does not generate new content; P1-1009 separately owns sanitization/full-solution leakage prevention. Domain and Workspace regression coverage plus final quality gates pass.
- [x] P1-1009 Prevent accidental full-solution leakage in learning mode. Progressive guidance now conservatively withholds revealed hint content containing fenced/code-like material or explicit full-solution framing, returns withheld-level metadata and exposes an explicit blocked learning-mode state. The original server-approved reveal flow remains unchanged. Leakage regressions and final quality gates pass.
- [x] P1-1010 Add AI request/error/cost observability. Applied migration `0014` adds durable owner-scoped metadata-only events. The real AI contest-draft call records success only after JSON/proposal/candidate validation and records provider, parsing and validation failures separately. Settings exposes recent private operation metadata without prompts/responses; provider cost is explicitly nullable when unavailable. Dedicated writer, protected-summary and Settings UI regressions plus final quality gates pass.
