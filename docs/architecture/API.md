...
| Settings           | `settings.get`, `update`, `setCodeforcesHandle`                    | Preferences are private and user-scoped; a Codeforces handle is stored only after an official public-profile lookup and never treated as ownership proof.    |
