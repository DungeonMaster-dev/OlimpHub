...
Первый опубликованный снимок `1.0.0` создан миграцией `0008_normal_snowbird`. Он привязывает 11 существующих утверждённых алгоритмических навыков к `skill_graph_versions` и не переписывает их stable key, описания или уже существующие связи. Защищённый `skills.map` выдаёт только узлы, связи и problem-link записи выбранной опубликованной версии и показывает semantic version с change summary в интерфейсе Skill Map. Расширение управления DAG (`P1-703`), математики (`P1-702`) и source-tag translation остаётся отдельной работой.
...
Карта навыков не хранит одно итоговое число как первичный факт. `SkillEvidence` выражает вклад конкретного события: решение задачи, попытка, использование подсказки, внешняя Accepted-отправка, время с последней практики. P1-704 определит детерминированную функцию, которая принимает только проверенные evidence и явную версию графа.
